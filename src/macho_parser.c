 ...
 *
 * This component uses open source code by Alex Denisov
...
int read_segment_commands(FILE *obj_file, int offset, int is_swap, uint32_t ncmds, struct entry_point_command* entry_point) {
    int actual_offset = offset;
    for (int  i = 0; i < ncmds; i++) {
        struct load_command *cmd = load_bytes(obj_file, actual_offset, sizeof(struct load_command));
        if (is_swap) {
...
int read_mach_header(FILE *obj_file, int offset, int is_64, int is_swap, struct entry_point_command* entry_point) {
    uint32_t ncmds;
    int load_commands_offset = offset;
    ...
    return read_segment_commands(obj_file, load_commands_offset, is_swap, ncmds, entry_point);
}
...
int read_entry_point_cmd(FILE *obj_file, struct entry_point_command* entry_point) {
    ...
    if (fat) {
        ...
        return read_mach_header(obj_file, 0, is_64, is_swap, entry_point);
    }
}
